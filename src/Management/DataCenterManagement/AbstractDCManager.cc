    ...
    if (DEBUG_CLOUD_SCHED) printf("\n Method[AbstractDCManager]: -------> initialize\n");
        ...
        // Get the topology of the data center!
            ...
            if (!configDone){
                ...
                if (topology == NULL) throw cRuntimeError ("AbstractDCManager::initialize() -> Error during initialization. There is no topology\n");
        ...
        // Get the time to log and scheduling events
            timeBetweenScheduleEvents_s  = par("timeBetweenScheduleEvents_s").longValue();
            timeBetweenLogResults_s = par("timeBetweenLogResults_s").doubleValue();

            if (timeBetweenScheduleEvents_s < 1) throw cRuntimeError ("The time between schedule events has to be at least 1..");
...
void AbstractDCManager::processRequestMessage (icancloud_Message *sm){
    throw cRuntimeError("AbstractDCManager::processRequestMessage->DataCenterManager does not process request messages\n");
}

void AbstractDCManager::processResponseMessage (icancloud_Message *sm){
...
void AbstractDCManager::notifyManager(icancloud_Message* msg){
        ...
        if (computeNodes){
            ...
            for (i = 0; i < nodesMapSize; i++){
                ...
                for (j = 0; j < nodeSetQuantity;j++){
                    ...
                    if (node == NULL) throw cRuntimeError ("DataCenterAPI::getEnergyConsumed->node is null after dynamic casting \n");
