...
void CloudSchedulerFCFS_gnup::initialize(){
    ...
    sprintf (log_name1, "%s/dataCenter_total_%04d-%02d-%02d_%02d:%02d:%02d.dat",OUT_DIR.c_str(), timeinfo->tm_year+1900,timeinfo->tm_mon+1,timeinfo->tm_mday,timeinfo->tm_hour,timeinfo->tm_min,timeinfo->tm_sec);
    csdcTot.Open(log_name1, par("outputCompression").boolValue());

    sprintf (log_name2, "%s/dataCenter_cont3DAggregated_%04d-%02d-%02d_%02d:%02d:%02d.dat",OUT_DIR.c_str(), timeinfo->tm_year+1900,timeinfo->tm_mon+1,timeinfo->tm_mday,timeinfo->tm_hour,timeinfo->tm_min,timeinfo->tm_sec);
    csdccont3dAg.Open(log_name2, par("outputCompression").boolValue());

    sprintf (log_name3, "%s/dataCenter_cont3D_%04d-%02d-%02d_%02d:%02d:%02d.txt",OUT_DIR.c_str(), timeinfo->tm_year+1900,timeinfo->tm_mon+1,timeinfo->tm_mday,timeinfo->tm_hour,timeinfo->tm_min,timeinfo->tm_sec);
    csdccont3D.Open(log_name3, par("outputCompression").boolValue());

    sprintf (log_name4, "%s/dataCenter_cont_%04d-%02d-%02d_%02d:%02d:%02d.txt",OUT_DIR.c_str(), timeinfo->tm_year+1900,timeinfo->tm_mon+1,timeinfo->tm_mday,timeinfo->tm_hour,timeinfo->tm_min,timeinfo->tm_sec);
...
vector<AbstractNode*> CloudSchedulerFCFS_gnup::selectStorageNodes (AbstractRequest* st_req){
    ...
    // Initialize ..
        ...
        if (streq == NULL) throw cRuntimeError("CloudSchedulerFSFC_gnup::selectStorageNodes ->can not cast to storage request\n");
...
int CloudSchedulerFCFS_gnup::selectNodeSet (string setName, int vmcpu, int vmmemory){
